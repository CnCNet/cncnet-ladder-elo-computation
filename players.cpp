...
void Players::add(const Player &player)
{
    ...
    {
        Log::warning() << "User id " << player.userId() << " already exists.";
    }
    ...
    gamemodes::GameMode gameMode) const
{
    ...
    // Maximum 30 players.
    for (size_t i = 0; i < std::max<size_t>(filteredAndSortedPlayers.size(), 30U); i++)
    {
        json jsonPlayer = json::object();
...
void Players::exportPlayerDetails(
    ...
    const std::map<uint32_t, Game*> &games) const
{
    using json = nlohmann::json;
