...
Rating::CalculationType Rating::update(const std::vector<std::array<double, 3>>& opponents, const std::vector<double>& results, CalculationType calculationType)
{
    ...
    /*if (_calculationType == CalculationType::Normal || calculationType == CalculationType::Normal)
    {
        updateNormally(opponents, results);
        return CalculationType::Normal;
    }
    else if (_calculationType == CalculationType::Initial || eloDeviation() > 200.0 && hasWinsAndLossesInResults(results))
    {
        // Moving from single calculation to special calculation.
        updateWithFirstWin(opponents, results, true);
        _calculationType = CalculationType::Normal;
        return CalculationType::Special;
    }
    else if (_calculationType == CalculationType::Initial || calculationType == CalculationType::SingleStep)
    {
        updateWithNoWin(opponents, results);
        _calculationType = CalculationType::SingleStep;
        return CalculationType::SingleStep;
    }
    else if ((_calculationType == CalculationType::SingleStep && hasWinsAndLossesInResults(results)) || calculationType == CalculationType::Special)
    {
        // Moving from single calculation to special calculation.
        updateWithFirstWin(opponents, results, false);
        _calculationType = CalculationType::Normal;
        return CalculationType::Special;
    }
    else
